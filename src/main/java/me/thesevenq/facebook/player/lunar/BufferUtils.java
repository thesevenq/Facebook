...
public class BufferUtils {
   public static byte[] writeVarInt(int value) throws IOException {
      ...
      for(os = new ByteArrayOutputStream(); (value & -128) != 0; value >>>= 7) {
   ...
   public static byte[] writeMap(Map players) throws IOException {
      ByteArrayOutputStream os = new ByteArrayOutputStream();
      os.write(writeVarInt(players.size()));
      Iterator var2 = players.entrySet().iterator();

      while(var2.hasNext()) {
         Entry player = (Entry)var2.next();
         os.write(getBytesFromUUID((UUID)player.getKey()));
         os.write(writeString((String)player.getValue()));
      }
